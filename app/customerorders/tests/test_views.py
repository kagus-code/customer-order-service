...
class TestCustomerAPI:
    ...
    def test_filter_customers(self, api_client, customer):
        response = api_client.get(f"/api/customers/?name={customer.name}")
        assert response.status_code == 200
    ...
    def test_invalid_customer_number(self, api_client):
        payload = {
            "name": "John Doe",
            "phone": "07012345672",
            "email": "user@example.com",
...
@pytest.mark.django_db
class TestOrderAPI:
    def test_list_orders(self, api_client, order):
        response = api_client.get("/api/customers/orders/")
        assert response.status_code == 200
        assert len(response.data["results"]) > 0

    def test_create_order(self, api_client, customer):
        payload = {
            "customer": customer.id,
            "order_items": [{"name": "string", "qty": 2, "price": 100}],
        }
        response = api_client.post(
            "/api/customers/orders/", payload, format="json"
        )

        assert response.status_code == 201

    def test_filter_orders(self, api_client, order):
        response = api_client.get(
            f"/api/customers/orders/?customer={order.customer.name}"
        )
        assert response.status_code == 200
        assert order.code in [ord["code"] for ord in response.data["results"]]

    def test_empty_name_order(self, api_client, customer):
        payload = {
            "customer": customer.id,
    ...
    def test_duplicate_item_name(self, api_client, customer):
        payload = {
            "customer": customer.id,
            "order_items": [
    ...
    def test_update_order_is_test(self, api_client, order):
        payload = {"is_paid": True}

        response = api_client.patch(
            f"/api/customers/orders/{order.id}/", payload, format="json"
        )
